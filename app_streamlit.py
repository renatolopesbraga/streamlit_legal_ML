...
st.write("""
         ...
         """
        """
        Inteligência Artificial que realiza o cálculo se um determinado processo judicial, 
...
st.header('CONFIRA OS DADOS INFORMADOS')
...
def prediction(pedido, valor_requerido, natureza_contigencia):
    ...
    st.write('O pedido informado foi: ', pedido)
    st.write('O valor requerido foi: ', valor_requerido)
    st.write('A natureza da contingência é: ', natureza_contigencia)

    if prediction == 0:
        pred = 'Desfavorável para o cliente'
    else:
        pred = 'Favorável para o cliente'
    return pred
